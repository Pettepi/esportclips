<?php
session_start();
session_save_path('/home2-3/p/pettepi/public_html/projekti/session');


$user = 'pettepi';        //Käytäjänimi
$pass = 'lollo123';        //Salasana, ei OMAn vaan phpAdminin
$host = 'mysql.metropolia.fi';  //Tietokantapalvelin
$dbname = 'pettepi';        //Tietokanta
...
//SQL statement being executed. All values are treated as PDO::PARAM_STR
    $data = array('varpusparvi' => $user, 'PWD' => $hashpwd);
//print_r($data);
    try {
//print_r($data);
//echo "Login 1<br />";
        $STH = $DBH->prepare("SELECT * FROM USERS WHERE EMAIL=:varpusparvi AND
PWD = :PWD");
